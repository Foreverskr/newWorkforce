...
export async function sendInactivityEmail(
  employee,
  { daysSinceAttendance, lastAttendanceDate } = {},
) {
  const formattedDate = lastAttendanceDate
    ? new Date(lastAttendanceDate).toLocaleDateString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  return sendEmail({
    to: employee.email,
    subject: "Attendance Status Notification",
    html,
  });
}
...
export async function sendLeaveApprovedEmail({
  ...
  return sendEmail({
    to,
    subject: `AttendTrack: Leave approved for ${employeeName}`,
    html,
  });
}
